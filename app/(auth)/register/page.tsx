    ...
    const [state, registerAction, isPending] = useActionState(
        async (prevState: RegisterState, formData: FormData): Promise<RegisterState> => {
            ...
            try {
                ...
                return {
                    error: error instanceof Error ? error.message : 'Registeration failed'
                }
            }
        },
        {
            ...
            <form action={registerAction}>
                <div className="text-center  mb-8">
                    ...
                    <p className="mt-2 text-sm text-slate-400">
                        Or  <Link className='font-medium text-blue-400 hover:text-blue-300' href={"/login"}>Sign in to existing account</Link>
                    </p>
                </div>
                {
                    state.error && (
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <input type="text" id='name' name='name' autoComplete='name' required placeholder='Enter your full Name' className='w-full px-3 py-2 bg-slate-900 border border-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 rounded' />
                    </div>
                    <div>
                        <label htmlFor="" className="block text-sm font-medium text-slate-300 mb-1">
                            Email
                    ...
                    <div>
                        ...
                        <input type="text" id='teamCode' name='teamCode' autoComplete='teamCode'  className='w-full px-3 py-2 bg-slate-900 border border-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 rounded' placeholder='Enter your team code'/>
                        <p className='text-xs text-slate-500 mt-1'>Leave if you dont have team code</p>
                    </div>
                </div>
